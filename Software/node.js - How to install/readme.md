...
This command downloads the `node-v0.10.27-linux-x64.tar.gz` and then saves it as `node.tar.gz` in your servers root directory.
...
~~~
cd && rm -rf node{.js.tar.gz,-v0.10.27-linux-x64}
~~~
...
~~~
wget -qO ~/node.tar.gz http://nodejs.org/dist/v0.10.27/node-v0.10.27.tar.gz
tar xf ~/node.tar.gz && cd ~/node-v0.10.27
./configure --prefix=$HOME
make && make install && cd
rm -rf ~/{node-v0.10.27,node.tar.gz}
~~~
