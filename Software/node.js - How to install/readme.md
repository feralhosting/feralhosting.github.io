...
This command downloads the `node-v0.10.29-linux-x64.tar.gz` and then saves it as `node.tar.gz` in your servers root directory.
...
~~~
cd && rm -rf node{.js.tar.gz,-v0.10.29-linux-x64}
~~~
...
~~~
wget -qO ~/node.tar.gz http://nodejs.org/dist/v0.10.29/node-v0.10.29.tar.gz
tar xf ~/node.tar.gz && cd ~/node-v0.10.29
./configure --prefix=$HOME
make && make install && cd
rm -rf ~/node{-v0.10.29,.tar.gz}
~~~
